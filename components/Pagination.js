...
export default class Pagination extends Component {
  ...
  mounted() {
    ...
    const renderButtons = (page) => {
      ...
      for (
        let i = startPage;
        i < startPage + buttonsPerPage && i < requiredPage;
        i++
      ) {
    ...
    const renderPage = (page) => {
      ...
      for (
        let i = startRecipeNumber;
        i < startRecipeNumber + recipesPerPage && i < numberOfRecipe;
        i++
      ) {
        const rcp = document.createElement("div");
        new ResultItem(rcp, { recipe: recipes[i], pagination: currentPage });
